...
export const PracticeCodingPanel = ({
  ...
  const renderProblemInfo = () => (
    <>
      ...
      <h2 className="text-lg font-semibold leading-tight">{question.title}</h2>
  ...
  const renderLanguageAndActions = () => (
  ...
  const renderResultsTabs = (tabsContentClassName: string) => (
    <Tabs
      value={activeTab}
    ...
    >
      ...
      <TabsContent value="tests" className={tabsContentClassName}>
              ...
              >
                ...
                {!r.passed && (
                  <div className="font-mono text-xs text-muted-foreground pl-6 space-y-0.5">
                    ...
                    <div>Got: {r.stdout || r.error || "(no output)"}</div>
                  </div>
                )}
              </div>
            ))}
      ...
      <TabsContent value="hints" className={tabsContentClassName}>
        {hints.length === 0 ? (
